...
interface AddPracticeChildModalProps {
  onClose: () => void;
  onAdd: (child: PracticeChild) => void;
}
...
export default function PracticeChildAdd({
  onClose,
  onAdd,
}: AddPracticeChildModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const created = await createPracticeChild({
      ...
      toast.success("ĐÃ TẠO PRACTICE CHILD THÀNH CÔNG", {
      ...
      toast.error("Thêm practice child thất bại", { description: message });
    } finally {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#1c1c1e] rounded-xl w-full max-w-7xl h-[90vh] relative flex flex-col">
        ...
        <div className="sticky top-0 z-10 bg-[#1c1c1e] p-6 border-b border-white/10">
          <h2 className="text-2xl font-bold text-white">Thêm practice child</h2>
          <button
