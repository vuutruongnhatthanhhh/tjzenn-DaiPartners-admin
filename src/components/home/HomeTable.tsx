// src/components/home/HomeTable.tsx
"use client";
...
export default function HomeTable() {
  ...
  const fetchItems = async () => {
    try {
      ...
      console.error("Lỗi khi fetch home:", err);
      toast.error("Không tải được danh sách home");
    }
  };

  const handleDelete = async (item: Home) => {
    const name = t(item.content) || "(không có nội dung)";
    const ok = window.confirm(`Xóa home: "${name}" ?`);
    if (!ok) return;

    try {
      await deleteHome(item.id as number);
      toast.success("Đã xóa home", { description: name });
      ...
      console.error("Delete home error:", message);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f0f10] text-white p-6 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        >
          <Plus className="w-4 h-4" /> Thêm home
        </button>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mt-4 items-center">
        <input
          ...
          className="px-3 py-2 rounded-lg bg-black text-white border border-gray-600 w-[420px]"
        />
      </div>
          ...
          <tbody>
            {items.length === 0 ? (
              ...
              items.map((h) => (
                ...
                >
                  <td className="px-4 py-3 rounded-l-xl max-w-[600px] truncate">
                    {t(h.content)}
                  </td>
