...
import { createOffice, type Office, type I18N } from "@/services/OfficeService";

interface OfficeAddProps {
  onClose: () => void;
...
export default function OfficeAdd({ onClose, onAdd }: OfficeAddProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);

  const [title, setTitle] = useState<I18N>({ ...emptyI18N });
  const [address, setAddress] = useState<I18N>({ ...emptyI18N });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("ĐÃ TẠO OFFICE THÀNH CÔNG", {
        description: (
          <strong>
            {t(title)} {t(address) ? `— ${t(address)}` : ""}
          </strong>
        ),
      });
      ...
      toast.error("Thêm office thất bại", { description: message });
    } finally {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#1c1c1e] rounded-xl w-full max-w-5xl h-[85vh] relative flex flex-col">
        <div className="sticky top-0 z-10 bg-[#1c1c1e] p-6 border-b border-white/10">
          <h2 className="text-2xl font-bold text-white">Thêm office</h2>
          <button
        ...
        >
          {/* ✅ Image */}
          <div>
            <label className="block mb-1 text-white">Ảnh văn phòng</label>
            ...
            {image ? (
              ...
              <p className="text-sm text-gray-400 mb-2">
                Chưa chọn ảnh. Nhấn “+ Chọn ảnh từ thư viện”.
              </p>
            )}
