...
interface AwardUpdateProps {
  award: Award;
  onClose: () => void;
  onUpdate: (updated: Award) => void;
}
...
const normalizeItem = (it?: Partial<AwardItem>): AwardItem => ({
...
export default function AwardUpdate({
  ...
  const [content, setContent] = useState<I18N>(
    award.content || { ...emptyI18N }
  );
  const [items, setItems] = useState<AwardItem[]>(
    Array.isArray(award.items) && award.items.length > 0
      ? award.items.map((x) => normalizeItem(x))
      : [normalizeItem()]
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updated = await updateAward(award.id as number, payload);

      toast.success("Cập nhật awards thành công");
      ...
      console.error("Update award error:", message);
    } finally {
